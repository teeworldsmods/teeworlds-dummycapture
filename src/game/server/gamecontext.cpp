	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			else if(str_comp_nocase(pMsg->m_Type, "kick") == 0)
			{
				...
				if(m_apPlayers[KickID] && m_apPlayers[KickID]->m_IsDummy)
				{
					SendChatTarget(ClientID, "You can't kick dummy");
					return;
				}
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Only %d active players are allowed", Server()->MaxClients()-g_Config.m_SvSpectatorSlots-1);
				SendBroadcast(aBuf, ClientID);
			}
		}
